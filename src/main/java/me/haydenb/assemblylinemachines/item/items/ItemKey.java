...
public class ItemKey extends Item {
				...
				if(!world.isClientSide && player.isShiftKeyDown()) {
					if(context.getLevel().getBlockEntity(context.getClickedPos()) instanceof AbstractMachine) {
						...
						if(key != null) {
							...
							is.setTag(nbt);
							player.displayClientMessage(new TextComponent("Key cut."), true);
						}else {
							player.displayClientMessage(new TextComponent("Could not cut Key."), true);
						}
	...
	public InteractionResultHolder<ItemStack> use(Level worldIn, Player playerIn, InteractionHand handIn) {
		...
		if(!worldIn.isClientSide && playerIn.isShiftKeyDown() && is.hasTag() && is.getTag().contains("assemblylinemachines:lockcode")) {
			...
			is.setTag(null);
			playerIn.displayClientMessage(new TextComponent("Cleared Key."), true);
	...
	public void appendHoverText(ItemStack stack, Level worldIn, List<Component> tooltip, TooltipFlag flagIn) {
		...
		if(stack.hasTag() && stack.getTag().contains("assemblylinemachines:lockcode")) {
			...
			if(s.trim().equals("")) {
				tooltip.add(1, new TextComponent("This Key is cut to a machine.").withStyle(ChatFormatting.DARK_GRAY));
			}else {
				tooltip.add(1, new TextComponent("This Key is cut to " + new TranslatableComponent(s).getString() + ".").withStyle(ChatFormatting.DARK_GRAY));
			}
			
			tooltip.add(2, new TextComponent("SHIFT-RIGHT CLICK again to reset the Key.").withStyle(ChatFormatting.DARK_GRAY));
		}else {
			tooltip.add(1, new TextComponent("This Key is not cut.").withStyle(ChatFormatting.DARK_GRAY));
			tooltip.add(2, new TextComponent("SHIFT-RIGHT CLICK a machine to cut the Key to the machine.").withStyle(ChatFormatting.DARK_GRAY));
		}
	}
}
