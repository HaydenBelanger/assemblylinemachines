...
import me.haydenb.assemblylinemachines.util.ICrankableMachine.ICrankableItem;
...
public class CrankTool<A extends TieredItem> extends TieredItem implements ICrankableItem{

	private final A parent;
	private final int maxCranks;
	public CrankTool(float damage, float speed, ToolType tt, Item.Properties builder, int maxCranks, A parent) {
	 ...
	 * WARNING: A MUST(!!!!!!) be either SwordItem, PickaxeItem, ShovelItem, HoeItem, or AxeItem.
	 * @return Made CrankTool
	 */
	@SuppressWarnings("unchecked")
	public static <A extends TieredItem> CrankTool<A> makeCrankTool(IItemTier tier, @Nullable ToolType tt, int attackDamage, float attackSpeed, Item.Properties props, int maxCranks, Class<A> clazz) {
		if(tt != null) {
		...
		try {
			for(Constructor<?> c : clazz.getConstructors()) {
				if(c.getParameterCount() == 4) {
					return new CrankTool<A>((float) attackDamage, attackSpeed, tt, props, maxCranks, (A) c.newInstance(tier, attackDamage, attackSpeed, props));
				}else {
					return new CrankTool<A>((float) attackDamage, attackSpeed, tt, props, maxCranks, (A) c.newInstance(tier, attackSpeed, props));
				}
			}
