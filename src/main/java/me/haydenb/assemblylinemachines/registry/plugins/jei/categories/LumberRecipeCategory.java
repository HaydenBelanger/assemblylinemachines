...
public class LumberRecipeCategory implements IRecipeCategory<LumberCrafting> {
	...
	public LumberRecipeCategory(IGuiHelper helper) {
		background = helper.createDrawable(JEIHelper.getGUIPath("gui_set_a"), 145, 69, 73, 50);
		icon = helper.createDrawableIngredient(new ItemStack(Registry.getBlock("lumber_mill")));
		progbar = helper.drawableBuilder(JEIHelper.getGUIPath("gui_set_a"), 218, 69, 19, 5).buildAnimated(200, StartDirection.LEFT, false);
	...
	public Component getTitle() {
		return new TextComponent("Lumber Mill Crafting");
	}
	...
	@Override
	public void draw(LumberCrafting recipe, PoseStack mx, double mouseX, double mouseY) {
	...
	public void setIngredients(LumberCrafting recipe, IIngredients ingredients) {
		...
		ingredients.setOutputs(VanillaTypes.ITEM, Arrays.asList(new ItemStack[] {recipe.getResultItem(), recipe.getSecondaryOutput()}));
