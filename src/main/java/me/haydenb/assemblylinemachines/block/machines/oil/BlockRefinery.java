...
public class BlockRefinery extends BlockScreenTileEntity<TERefinery> {
	...
	private static final VoxelShape SHAPE_N = Stream.of(
			Block.makeCuboidShape(3, 5, 4, 13, 10, 11),
			Block.makeCuboidShape(3, 5, 2, 5, 10, 4),
			Block.makeCuboidShape(11, 5, 2, 13, 10, 4),
			Block.makeCuboidShape(9, 6, 2, 10, 8, 3),
			Block.makeCuboidShape(6, 6, 2, 7, 8, 3),
			Block.makeCuboidShape(5, 8, 2, 11, 10, 4),
			Block.makeCuboidShape(5, 5, 2, 11, 6, 4),
			Block.makeCuboidShape(5, 6, 3, 11, 8, 3),
			Block.makeCuboidShape(1, 5, 6, 6, 13, 11),
			Block.makeCuboidShape(10, 5, 6, 15, 13, 11),
			Block.makeCuboidShape(3, 10, 5, 4, 14, 6),
			Block.makeCuboidShape(12, 10, 5, 13, 14, 6),
			Block.makeCuboidShape(3, 13, 6, 4, 14, 7),
			Block.makeCuboidShape(3, 13, 10, 4, 14, 11),
			Block.makeCuboidShape(12, 13, 6, 13, 14, 7),
			Block.makeCuboidShape(12, 13, 10, 13, 14, 11),
			Block.makeCuboidShape(11, 13, 7, 14, 14, 10),
			Block.makeCuboidShape(2, 13, 7, 5, 14, 10),
			Block.makeCuboidShape(12, 13, 8, 13, 16, 9),
			Block.makeCuboidShape(3, 13, 8, 4, 16, 9),
			Block.makeCuboidShape(0, 0, 0, 16, 5, 16),
			Block.makeCuboidShape(0, 5, 11, 16, 16, 16)
	...
	public BlockRefinery() {
		super(Block.Properties.create(Material.IRON).hardnessAndResistance(4f, 15f).harvestLevel(0)
				.harvestTool(ToolType.PICKAXE).sound(SoundType.METAL), "refinery", BlockRefinery.TERefinery.class);
	...
	public void animateTick(BlockState stateIn, World world, BlockPos pos, Random rand) {
		...
		if(stateIn.get(StateProperties.MACHINE_ACTIVE)) {
			...
			world.addParticle(ParticleTypes.LARGE_SMOKE, true, pos.getX(), pos.getY(), pos.getZ(), 0, 0, 0);
		}
	...
	public static class TERefinery extends ManagedSidedMachine<ContainerRefinery> implements ITickableTileEntity{
						...
						if(amount - cost >= 0) {
							if(progress >= cycles) {
								...
								if(!tempa.getFirst().isEmpty()) {
									...
									if(contents.get(0).isEmpty() || (ItemHandlerHelper.canItemStacksStack(contents.get(0), outstack) && contents.get(0).getCount() + outstack.getCount() <= contents.get(0).getMaxStackSize())){
										stackOut = outstack;
									}else {
										return;
									}
		...
		public TERefinery(final TileEntityType<?> tileEntityTypeIn) {
			super(tileEntityTypeIn, 5, (TranslationTextComponent) Registry.getBlock("refinery").getNameTextComponent(), Registry.getContainerId("refinery"), ContainerRefinery.class, new EnergyProperties(true, false, 160000));
		}
	...
	public static class ScreenRefinery extends ScreenALMEnergyBased<ContainerRefinery>{
		...
		public ScreenRefinery(ContainerRefinery screenContainer, PlayerInventory inv,
				ITextComponent titleIn) {
			super(screenContainer, inv, titleIn, new Pair<>(175, 165), new Pair<>(11, 6), new Pair<>(11, 73), "refinery", false, new Pair<>(14, 17), screenContainer.tileEntity, true);
		...
		private void renderFluidTooltip(FluidStack fs, int mouseX, int mouseY, int mminx, int mminy, int bx, int by, boolean cm) {
			...
			if(mouseX >= mminx && mouseY >= mminy && mouseX <= mminx + 7 && mouseY <= mminy + 36) {
				if(!fs.isEmpty()) {
					...
					str.add(fs.getDisplayName().getFormattedText());
